...
//#define BTRLZ_HASH_SZ	1024
#define BTRLZ_HASH_SZ	4096
#define BTRLZ_HASH_DN	64
#define BTRLZ_HASH_DN1	63

#define BTRLZ_CHHASH_SZ	16384
#define BTRLZ_CHAIN_SZ	65536
...
byte	*chn_base;
byte	*chn_ptrs[BTRLZ_CHAIN_SZ];
u16		chn_next[BTRLZ_CHAIN_SZ];
u16		chn_hash[BTRLZ_CHHASH_SZ];

byte *hash[BTRLZ_HASH_SZ*BTRLZ_HASH_DN];
byte hrov[BTRLZ_HASH_SZ];
byte *cs;
byte *ct;
int maxlen;
int maxdist;
...
int BTRLZ0_HashByte(BTRLZ0_EncContext *ctx, byte *cs)
{
	...
	rv=(cs-ctx->chn_base)&(BTRLZ_CHAIN_SZ-1);

	h=BTRLZ0_CalcHash(cs);
	
	hr=ctx->hrov[h];
	hr=(hr-1)&BTRLZ_HASH_DN1;
	ctx->hrov[h]=hr;
	ctx->hash[(h*BTRLZ_HASH_DN)+hr]=cs;
...
//	cse=str+1023;
	...
	for(i=0; i<BTRLZ_HASH_DN; i++)
	{
		cs1=ctx->hash[(h*BTRLZ_HASH_DN)+((hr+i)&BTRLZ_HASH_DN1)];
		if(!cs1)continue;
...
int BTRLZ0_EncodeBuffer(BTRLZ0_EncContext *ctx,
	byte *ibuf, byte *obuf, int ibsz, int obsz, int lvl)
{
	byte *cs, *cse, *lcs;
	byte *ct;
	u32 v;
...
//		ctx->stat_rlen[btrlz0_log2u(rl)]++;
//		ctx->stat_len[btrlz0_log2u(l)]++;
//		ctx->stat_dist[btrlz0_log2u(d)]++;
...
int BTRLZ0_EncodeBufferNoCtx(
	byte *ibuf, byte *obuf, int ibsz, int obsz, int lvl)
{
	static BTRLZ0_EncContext *ctx=NULL;
	
	if(!ctx)
		ctx=malloc(sizeof(BTRLZ0_EncContext));
	return(BTRLZ0_EncodeBuffer(ctx, ibuf, obuf, ibsz, obsz, lvl));
}
