...
int BT5BU_InitTables()
{
	...
	static byte remap4dxt_a[4]={0, 3, 3, 1};
	static byte remap4dxt_b[4]={0, 3, 2, 2};
	u32 pix;
	int i0, i1, i2, i3, px;
	int i, j, k;
	...
	for(i=0; i<256; i++)
	{
		...
		j=	(remap4dxt_b[(i>>0)&3]<<0) |
			(remap4dxt_b[(i>>2)&3]<<2) |
			(remap4dxt_b[(i>>4)&3]<<4) |
			(remap4dxt_b[(i>>6)&3]<<6) ;
		bt5bu_conv4x2dxtea[i]=j;
	}
