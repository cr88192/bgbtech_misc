...
#if 1
			bclr[1]=(CRAM_GETU16(cs+ 0))&32767;
			bclr[0]=(CRAM_GETU16(cs+ 2))&32767;
			cclr[1]=(CRAM_GETU16(cs+ 4))&32767;
			cclr[0]=(CRAM_GETU16(cs+ 6))&32767;
			dclr[1]=(CRAM_GETU16(cs+ 8))&32767;
			dclr[0]=(CRAM_GETU16(cs+10))&32767;
#endif
...
//	memset(dst, 255, info->ohead->biWidth*info->ohead->biHeight*4);
	...
	}else if(dst && (clrs==BGBBTJ_JPG_BGRA))
	{
...
//		cram_decompress_inner_rcon(src, info->buffer, ssz, dsz, info);
	}else if(clrs==BGBBTJ_JPG_RAWDAT)
	{
//		cram_decompress_inner_rcon(src, info->buffer, ssz, dsz, info);
		memset(dst, 0, ssz+16);
		memcpy(dst, src, ssz);
