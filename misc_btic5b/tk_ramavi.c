...
#define BGBBTJ_JPG_RGB555 18

#define BGBBTJ_JPG_RAWCON 64
#define BGBBTJ_JPG_RAWDAT 65
...
//	tmp=bgbrpi_talloc("lbxgl_vidcodec_t", sizeof(BGBBTJ_VidCodec));
	tmp=pdgl_avi_malloc(sizeof(BGBBTJ_VidCodec));
...
//	tmp=bgbrpi_talloc("lbxgl_vidcodecctx_t", sizeof(BGBBTJ_VidCodecCTX));
	tmp=pdgl_avi_malloc(sizeof(BGBBTJ_VidCodecCTX));
	return(tmp);
}
...
//	info=bgbrpi_malloc(sizeof(vfw_ctxinfo));
	info=pdgl_avi_malloc(sizeof(vfw_ctxinfo));
	ctx->data=info;

	info->ihead=pdgl_avi_malloc(sizeof(BGBBTJ_BITMAPINFOHEADER));
	...
	info->ohead=pdgl_avi_malloc(sizeof(BGBBTJ_BITMAPINFOHEADER));
...
//	info->ohead->biHeight		= -out->biHeight;
	...
	info->buffer=pdgl_avi_malloc(out->biWidth*out->biHeight*out->biBitCount/8);
...
#if 0
	info=pdgl_avi_malloc(sizeof(ICINFO));
...
//	ctx=bgbrpi_talloc("lbxgl_avi_t", sizeof(BGBBTJ_AVI_Context));
	ctx=pdgl_avi_malloc(sizeof(BGBBTJ_AVI_Context));
	...
	default:
		...
		inhead=pdgl_avi_malloc(sizeof(BGBBTJ_BITMAPINFOHEADER));
		outhead=pdgl_avi_malloc(sizeof(BGBBTJ_BITMAPINFOHEADER));
		inhead->biWidth=w;
		inhead->biHeight=h;
	...
	ctx->fdbuf=pdgl_avi_malloc(w*h*4);
	ctx->fadbuf=pdgl_avi_malloc(16384);
	...
	ctx->sampbuf=pdgl_avi_malloc(1<<18);
...
BGBBTJ_Video_Stats *BGBBTJ_AVI_GetStats(BGBBTJ_AVI_Context *ctx)
{
	...
	tmp=pdgl_avi_malloc(sizeof(BGBBTJ_Video_Stats));
