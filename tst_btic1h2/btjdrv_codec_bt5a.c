static int bt5a_decompress_frame_clrs(BTIC1H_VidCodecCTX *ctx,
...
//	BTPIC0A_DecodeFrame(info->vctx, src, ssz, dst,
//		info->ihead->biWidth, clrs);

	BTPIC0A_DecodeFrame(info->vctx, src, ssz, ibuf,
		ystr, clrs);

	return(0);
}

static int bt5a_decompress_frame(BTIC1H_VidCodecCTX *ctx,
...
int bt5a_end_decompress(BTIC1H_VidCodecCTX *ctx)
{
	btic5a_ctxinfo *info;
...
//	BTPIC0A_FreeDecodeContext(ctx->data);
	BTPIC0A_FreeEncodeContext(info->vctx);
	free(info);
	return(0);
}

static BTIC1H_VidCodecCTX *bt5a_begin_decompress(int fcc,
	BTIC1H_BMPInfoHeader *in, BTIC1H_BMPInfoHeader *out)
{
	...
	if(in->biCompression!=BTIC1H_FCC_bt5a)
		return(NULL);
...
//			{ info->clrs=BTIC4B_CLRS_BC6H; }
	...
	ctx->end_decompress=bt5a_end_decompress;

	return(ctx);
}

int bt5a_decompress_query(int fcc,
	BTIC1H_BMPInfoHeader *in, BTIC1H_BMPInfoHeader *out)
{
	int err;

	if((fcc!=BTIC1H_FCC_bt5a) && (in->biCompression!=BTIC1H_FCC_bt5a))
		return(0);
	return(1);
}


int bt5a_compress_frame(BTIC1H_VidCodecCTX *ctx,
...
//		src, info->xs, info->ys, qfl1, clrs1);
	...
	sz=BTPIC_EncodeFrameImage(info->vctx, dst, dsz,
		src, qfl1, clrs1);
...
int bt5a_end_compress(BTIC1H_VidCodecCTX *ctx)
{
	btic5a_ctxinfo *info;
...
BTIC1H_VidCodecCTX *bt5a_begin_compress(int fcc,
	BTIC1H_BMPInfoHeader *in, BTIC1H_BMPInfoHeader *out)
{
	BTIC1H_VidCodecCTX *ctx;
	BTPIC0A_EncodeContext *btctx;
	btic5a_ctxinfo *info;
	int err;

	if((fcc!=BTIC1H_FCC_bt5a) && (out->biCompression!=BTIC1H_FCC_bt5a))
		return(NULL);
...
//	ctx->data=info;
	...
	btctx=BTPIC0A_AllocEncodeContext();
	info->vctx=btctx;
...
//	info->ys=abs(in->biHeight);
	...
	ctx->compress_frame=&bt5a_compress_frame;
	ctx->end_compress=bt5a_end_compress;

	return(ctx);
}

int bt5a_compress_query(int fcc,
	BTIC1H_BMPInfoHeader *in, BTIC1H_BMPInfoHeader *out)
{
	int err;
...
int BTIC1H_CodecBT5A_Init()
{
	...
	codec->begin_compress=&bt5a_begin_compress;

	codec->decompress_query=&bt5a_decompress_query;
	codec->compress_query=&bt5a_compress_query;
