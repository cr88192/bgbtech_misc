typedef struct LQTVQ_ImageSlice_s LQTVQ_ImageSlice;

struct LQTVQ_ImageSlice_s {
LQTVQ_ImageSlice *next;
BT4A_Context tctx;
byte *ibuf;
byte *blks;
byte *lblks;
...
volatile LQTVQ_ImageSlice *lqtvq_slicelive=NULL;
...
{
	struct LQTVQ_ImageSlice_s *slc;
	
	slc=ptr;
	LQTVQ_EncImageBGRA(&slc->tctx, slc->blks, slc->ibuf, slc->xs, slc->ys);
	slc->done=1;
	return(0);
}
...
int lqtvq_sliceWorker(void *ptr)
{
	...
	while(1)
	{
		...
		slc=(LQTVQ_ImageSlice *)lqtvq_slicelive;
		if(slc)
		...
		ctx=&slc->tctx;
		LQTVQ_SetupContextQf(ctx, slc->qf);
		LQTVQ_EncImageBGRA(ctx,
			slc->blks, slc->ibuf, slc->xs, slc->ys);
		LQTVQ_EncImgBlocks(ctx, slc->obuf,
			slc->blks, slc->lblks,
...
int LQTVQ_EncImageThreadsBGRA(BT4A_Context *ctx,
	byte *obuf, int obmsz,
	byte *ibuf, int xs, int ys, int qf)
{
	static LQTVQ_ImageSlice slice[8];
	LQTVQ_ImageSlice *slc;
	byte *cs, *ct;
	...
	for(i=0; i<8; i++)
	{
		...
		memcpy(&slice[i].tctx, ctx, sizeof(BT4A_Context));
		...
		slc->next=(LQTVQ_ImageSlice *)lqtvq_slicelive;
		lqtvq_slicelive=slc;
	}
	thUnlockFastMutex(lqtvq_slicemutex);
