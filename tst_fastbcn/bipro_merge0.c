...
//#ifdef BIPRO_DLL
#if 1
#define BIPRO_API __declspec(dllexport)
#else
#define BIPRO_API __declspec(dllimport)
#endif
#else
#define BIPRO_API
#endif
...
typedef struct BIPRO_ExceptionType_s BIPRO_ExceptionType;
typedef struct BIPRO_Exception_s BIPRO_Exception;

typedef struct BIPRO_Job_s BIPRO_Job;
typedef struct BIPRO_WorkerInfo_s BIPRO_WorkerInfo;

typedef BIPRO_Exception *gcehException;
...
struct BIPRO_ExceptionType_s {
BIPRO_ExceptionType *next;
BIPRO_ExceptionType *parent;
char *name;
};

struct BIPRO_Exception_s {
BIPRO_ExceptionType *type;	//type of exception
BIPRO_Exception *chain;		//chained exception (rethrows)
...
struct BIPRO_WorkerInfo_s {
int workerId;					//worker ID (for special workers)
void (*begin)(BIPRO_WorkerInfo *info);
void (*end)(BIPRO_WorkerInfo *info);
};
...
volatile u32 *bipro_prof_stats_eip=NULL;
...
//	j=(1<<16)-256;
	...
	q=(void *)(((void **)p)-j);
//	BIPRO_ScanRange((void **)q, j);

//	if(inf->tls)
//		BIPRO_ScanRange(inf->tls, bipro_thread_tlsrov);
}
...
//	BIPRO_AddMarkHandler(bipro_thread_mark);

//	bipro_alloc_mutex=thMutex();
...
#if 0
// char *BIPRO_LookupModulePtrName(void *ptr)
{
	s64 va, vae;
	int i, j, k;
	...
	{
		va=bipro_mod_text[i];
		vae=va+bipro_mod_textsz[i];
		if(	(((byte *)ptr)>=((byte *)va)) &&
			(((byte *)ptr)<((byte *)vae)))
		{
			return(bipro_mod_name[i]);
		}

		va=bipro_mod_data[i];
		vae=va+bipro_mod_datasz[i];
		if(	(((byte *)ptr)>=((byte *)va)) &&
			(((byte *)ptr)<((byte *)vae)))
		{
			return(bipro_mod_name[i]);
...
//	if(bipro_lookup_blacklist(name)>0)
//		return(NULL);
//	if((*name=='_') && (bipro_lookup_blacklist(name+1)>0))
//		return(NULL);

//	printf("BIPRO_LookupLabelOS: module %p\n", bipro_hmodapp);

	p=BIPRO_LookupLabelOS_2(name, &fl);

#ifdef X86_64
//	if(p && (((s64)p)&0xFFFFFFFF00000000LL) )
	if(p && BIPRO_CheckPtrBigSpace(p))
	{
		BIPRO_AddProxyPtr(name, p);
		q=BIPRO_FetchSym(name);
//		printf("BIPRO_LookupLabelOS: Proxy %s->%p @%p\n", name, p, q);
		return(q);
	}
#endif
...
//	bipro_sym_name=malloc(i*sizeof(char *));
//	bipro_sym_addr=malloc(i*sizeof(void *));
//	bipro_sym_nsym=0;
//	bipro_sym_msym=i;
...
char *BIPRO_GetLastNamePtr(void *ptr, void **rbp)
{
	...
	if(!bp)
	{
		s=BIPRO_LookupAddrNameOS(ptr, rbp);
		if(s)return(s);
...
//		s=bipro_importfuncs.LookupSectionAddrName(ptr);
//		if(s)return(s);
//	}
	
//	if(BIPRO_GetChunk(ptr))
//		return("<HEAP>");
...
BIPRO_API int BIPRO_ProfilerSetActive(int fl)
{
	int i, j, k;
...
#if 0
int main_dum1, main_dum2;
BIPRO_API int main_dummy()
{
	main_dum1+=main_dum2;
	main_dum2+=main_dum1;
}

BIPRO_API int main_loop0()
{
	int i, j;
